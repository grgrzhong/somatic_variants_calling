...
export PURECN_REF_DIR="${REFERENCE_DIR}/PureCN_2"
...
export PURECN_DIR="${PROJECT_DIR}/data/DFSP/PureCN_2"
...
## Test Mutect2/GATK4 normal panel GenomicsDB - work
singularity exec \
    --bind ${PROJECT_DIR} \
    --bind ${REFERENCE_DIR} \
    ${CONTAINER_DIR}/purecn.sif \
...
purecn_analysis() {
    ...
    ## Use mutect2 VCF file: Error: No variants passed filter BQ.
    # singularity exec \
    #     --bind ${PROJECT_DIR} \
    #     --bind ${REFERENCE_DIR} \
    #     ${CONTAINER_DIR}/purecn.sif \
    ...
    ## Use final VCF file: Error: Segmentation and VCF do not overlap.
    singularity exec \
        --bind ${PROJECT_DIR} \
        --bind ${REFERENCE_DIR} \
        ${CONTAINER_DIR}/purecn.sif \
        Rscript "$PURE_PATH/PureCN.R" \
            ...
            --tumor "${PURECN_DIR}/${tumor_id}/${tumor_id}_recalibrated_coverage_loess.txt.gz" \
            --sampleid "${tumor_id}" \
            --vcf "${MUTECT2_DIR}/${tumor_id}/${tumor_id}.final.vcf.gz" \
            --normaldb "${PURECN_REF_DIR}/normalDB_hg38.rds" \
            --intervals "${PURECN_REF_DIR}/baits_hg38_intervals.txt" \
            --genome hg38 \
            >& "${PURECN_DIR}/${tumor_id}/${tumor_id}.PureCN.log"
